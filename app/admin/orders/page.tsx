...
export default function OrdersPage() {
    ...
    // 🚀 LECTURA DE PATRÓN MEMORIZADA PARA EVITAR DUPLICADOS DE PROCESADO
    ...
    return (
        <div className="min-h-screen bg-[#F6F6F6] pb-20 font-sans text-gray-900 flex flex-col">
            ...
            <div className="w-full max-w-[100vw] overflow-x-hidden flex-1">
                <div className="max-w-350 mx-auto px-4 md:px-8 py-8 space-y-6 md:space-y-8">
                    ...
                    {loading && orders.length === 0 ? (
                        ...
                        <>
                            {/* VISTA MÓVIL ACTUALIZADA CON RETENCIÓN */}
                                    ...
                                    <div key={order.id} onClick={() => openDrawer(order)} className="bg-white rounded-(--radius-card) border border-transparent hover:border-gray-200 p-4 active:bg-gray-50 transition-colors cursor-pointer w-full relative">
                                        <div className="flex justify-between items-start mb-3">
                                            <div className="min-w-0 pr-2">
                                               ...
                                               <div className="flex gap-1.5 flex-wrap">
                                                    {order.is_quote ? <span className="px-2 py-0.5 bg-purple-50 text-purple-700 text-[8px] font-black uppercase tracking-wider rounded-sm ">Cotización</span> :
                                                        order.source === 'pos' ? <span className="px-2 py-0.5 bg-blue-50 text-blue-700 text-[8px] font-black uppercase tracking-wider rounded-sm ">POS</span> :
                                                            <span className="px-2 py-0.5 bg-gray-50 text-gray-500 text-[8px] font-black uppercase tracking-wider rounded-sm ">Web</span>}
                                                    {order.payment_method && (
                                                        <span className="px-2 py-0.5 bg-slate-100 text-slate-600 text-[8px] font-black uppercase tracking-wider rounded-sm ">
                                                            {order.payment_method}
                                                        </span>
                                                    )}
                                                    
                                                    {/* 🚀 ETIQUETA CONTABLE DE VUELTO VIRTUAL EN MÓVIL (Sombras ultradifuminadas de alta gama) */}
                                                    {order.delivery_info?.includes('⚠️ VUELTO VIRTUAL') && (
                                                        order.vuelto_processed ? (
                                                            <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 text-[8px] font-black uppercase tracking-wider rounded-sm  shadow-[0_4px_12px_rgb(16,185,129,0.02)]">
                                                                Vuelto Otorgado
                                                            </span>
                                                        ) : (
                                                            <span className="px-2 py-0.5 bg-amber-50 text-amber-700 text-[8px] font-black uppercase tracking-wider rounded-sm shadow-[0_4px_12px_rgb(245,158,11,0.02)]">
                                                                Vuelto Pendiente
                                                            </span>
                                                        )
                                                    )}
                                        ...
                                        <div className="flex justify-between items-end mt-3 border-t border-gray-50 pt-2">
                                            <div className="text-right shrink-0 ml-auto">
                                                <p className="font-black text-base flex items-center justify-end gap-1.5 leading-none">
                                                    {Number(order.iva_retention_usd || 0) > 0 ? (
                                                        <span className="text-emerald-600">${Number(order.liquid_amount_usd).toFixed(2)}</span>
                                                    ) : (
                                                        <span className="text-gray-900">${Number(order.total_usd).toFixed(2)}</span>
                                                    )}
                                                    <span className="text-[8px] text-gray-500 px-1.5 py-0.5">{order.currency_type === 'eur' ? 'EUR' : 'USD'}</span>
                                                </p>
                                                {Number(order.iva_retention_usd || 0) > 0 && (
                                                    <p className="text-[9px] text-red-500 font-bold mt-1 text-right">Retenido: -${Number(order.iva_retention_usd).toFixed(2)}</p>
                                                )}
                            ...
                            {/* VISTA DESKTOP ACTUALIZADA CON RETENCIÓN */}
                                                ...
                                                <tr key={order.id} onClick={() => openDrawer(order)} className="hover:bg-gray-50/50 transition-colors cursor-pointer group">
                                                   <td className="px-6 py-4 whitespace-nowrap">
                                                        ...
                                                        <div className="flex gap-1.5 flex-wrap max-w-40">
                                                            {order.is_quote ? <span className="px-1.5 py-0.5 bg-purple-50 text-purple-700 text-[8px] font-black uppercase tracking-wider rounded-sm ">Cotización</span> :
                                                                order.source === 'pos' ? <span className="px-1.5 py-0.5 bg-blue-50 text-blue-700 text-[8px] font-black uppercase tracking-wider rounded-sm ">POS</span> :
                                                                    <span className="px-1.5 py-0.5 bg-gray-50 text-gray-500 text-[8px] font-black uppercase tracking-wider rounded-sm ">Web</span>}
                                                            {order.payment_method && (
                                                                <span className="px-1.5 py-0.5 bg-slate-100 text-slate-600 text-[8px] font-black uppercase tracking-wider rounded-sm">
                                                                    {order.payment_method}
                                                                </span>
                                                            )}
                                                            
                                                            {/* 🚀 ETIQUETA CONTABLE DE VUELTO VIRTUAL EN ESCRITORIO */}
                                                            {order.delivery_info?.includes('⚠️ VUELTO VIRTUAL') && (
                                                                order.vuelto_processed ? (
                                                                    <span className="px-1.5 py-0.5 bg-emerald-50 text-emerald-700 text-[8px] font-black uppercase tracking-wider rounded-sm  shadow-[0_4px_12px_rgb(16,185,129,0.02)]">
                                                                        Vuelto Otorgado
                                                                    </span>
                                                                ) : (
                                                                    <span className="px-1.5 py-0.5 bg-amber-50 text-amber-700 text-[8px] font-black uppercase tracking-wider rounded-sm  shadow-[0_4px_12px_rgb(245,158,11,0.02)]">
                                                                        Vuelto Pendiente
                                                                    </span>
                                                                )
                                                            )}
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-right">
                                                        <p className="font-black flex items-center justify-end gap-1.5">
                                                            {Number(order.iva_retention_usd || 0) > 0 ? (
                                                                <>
                                                                    <span className="text-gray-400 line-through text-[10px]">${Number(order.total_usd).toFixed(2)}</span>
                                                                    <span className="text-emerald-600">${Number(order.liquid_amount_usd).toFixed(2)}</span>
                                                                </>
                                                            ) : (
                                                                <span className="text-gray-900">${Number(order.total_usd).toFixed(2)}</span>
                                                        ...
                                                        {Number(order.iva_retention_usd || 0) > 0 && (
                                                            <p className="text-[9px] text-red-500 font-bold mt-0.5 text-right">Retenido: -${Number(order.iva_retention_usd).toFixed(2)}</p>
                                                        )}
                    ...
                    <div className="fixed inset-0 z-100 flex justify-end">
                        ...
                        <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="relative w-full md:w-112.5 bg-white h-full flex flex-col shadow-2xl">
                            ...
                            <div className="flex-1 overflow-y-auto p-6 pt-0 space-y-8 no-scrollbar">
                                <div className="mt-6 p-5 bg-[#FAFAFA]  rounded-(--radius-btn) flex flex-col gap-4">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 bg-white  rounded-(--radius-btn) flex items-center justify-center shrink-0">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500 font-medium mt-1.5 leading-tight">
                                                {selectedOrder.status === 'quote' ? 'Comparte el enlace para concretar la venta.' :
                                                    selectedOrder.status === 'pending' ? 'Concilia el pago para liberar la orden de pedido.' :
                                                        'Documento definitivo emitido y procesado.'}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex gap-2 mt-2">
                                        <button onClick={(e) => handleCopyQuote(e, selectedOrder.id)} className="flex-1 py-3 bg-white  rounded-xl text-[10px] font-bold uppercase tracking-widest hover:border-gray-300 transition-all flex items-center justify-center gap-1.5 text-gray-700 active:scale-95 shadow-md">
                                        ...
                                        {selectedOrder.split_payments && selectedOrder.split_payments.length > 0 ? (
                                            <div className="grid grid-cols-1 gap-2.5">
                                                {selectedOrder.split_payments.map((payment: any, index: number) => (
                                                    <div key={index} className="bg-gray-50 rounded-(--radius-btn) p-3 border border-gray-100 flex items-center justify-between gap-3">
                                                        <div className="flex items-center gap-3 min-w-0">
                                                            <div className="w-10 h-10 bg-white border border-gray-100 rounded-lg flex items-center justify-center shrink-0 overflow-hidden">
                                                                {payment.receipt_url ? (
                                                        ...
                                                        {payment.receipt_url && payment.receipt_url.startsWith('http') ? (
                                                            ...
                                                            <span className="shrink-0 px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-md text-[9px] font-black uppercase tracking-widest flex items-center gap-1">
                                            ...
                                            <div className="bg-gray-50 rounded-(--radius-card) p-2.5 border border-gray-100 flex items-center justify-between gap-3">
                                                <div className="flex items-center gap-3 overflow-hidden">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-[10px] font-medium text-gray-500 uppercase tracking-wider">Pago Único</p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="bg-gray-50 border border-gray-100 border-dashed rounded-xl p-4 flex flex-col items-center justify-center text-gray-400">
                                                ...
                                                <p className="text-[10px] font-bold uppercase tracking-widest text-center">Sin comprobante en BD</p>
                                            </div>
                                        )}
                                    </div>
                                )}
                                    ...
                                    <div className="min-w-0 pr-4">
                                        ...
                                        {selectedOrder.customer_phone && (
                                            <a href={`https://wa.me/${selectedOrder.customer_phone.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" 
                                              className="flex items-center gap-1.5 text-xs font-bold text-green-700 hover:text-green-800 mt-1 w-fit bg-green-50 px-2.5 py-1.5 rounded-(--radius-badge) truncate transition-colors"
                                            >
                                              <WhatsAppIcon size={14} className="shrink-0 fill-current" /> 
                                              <span className="truncate">{selectedOrder.customer_phone}</span>
                                            </a>
                                        )}
                                    </div>

                                    {/* 🚀 AQUÍ ESTÁ EL BLOQUE DEL DESGLOSE FINAL CORREGIDO */}
                                    <div className="text-right shrink-0">
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Total Facturado</p>
                                        <p className={`font-black text-2xl leading-none ${Number(selectedOrder?.iva_retention_usd || 0) > 0 ? 'text-gray-400 line-through' : 'text-gray-900'}`}>
                                        ...
                                        {Number(selectedOrder?.iva_retention_usd || 0) > 0 && (
                                            <div className="mt-2 text-right animate-in fade-in slide-in-from-top-2">
                                                <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest mb-0.5">Retenido ({selectedOrder?.iva_retention_pct}%)</p>
                                                <p className="font-bold text-sm text-red-500 leading-none">-${Number(selectedOrder?.iva_retention_usd || 0).toFixed(2)}</p>

                                                <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mt-2 mb-0.5">Monto Líquido (A cobrar)</p>
                                                <p className="font-black text-2xl text-emerald-600 leading-none">${Number(selectedOrder?.liquid_amount_usd || 0).toFixed(2)}</p>
                                            </div>
                                        )}

                                        <p className="text-[10px] font-mono font-bold text-gray-400 mt-2 text-right">Bs {getBsAmount(selectedOrder || {}).toLocaleString('es-VE', { maximumFractionDigits: 2 })}</p>
                                       
                                    </div>
                                </div>
                                <div className="bg-gray-50 rounded-(--radius-btn) p-6 space-y-4 border border-gray-100">
                                        
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Dirección de Entrega</p>
                                    <div className="flex items-start gap-3 bg-white p-4 rounded-(--radius-btn) shadow-[0_8px_30px_rgb(0,0,0,0.02)] border border-gray-50">
                                        <MapPin size={16} className="text-gray-400 shrink-0 mt-0.5" />
                                        <p className="text-sm font-medium text-gray-700 leading-snug flex-1 wrap-break-word">
                                            {selectedOrder.delivery_info?.split(' | ⚠️ ')[0] || 'Retiro en Tienda'}
                                        </p>
                                    </div>
                                    
                                    {/* 🚀 ALERTA INTELIGENTE Y BOTÓN CONTEXTUAL */}
                                    {(() => {
                                        if (selectedOrder.vuelto_processed) {
                                            return (
                                                <div className="mt-4 p-5 bg-white  rounded-(--radius-btn) shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex items-start gap-3">
                                                    <div className="p-2 bg-emerald-50 text-emerald-700 rounded-full shrink-0">
                                                        <Check size={16} strokeWidth={3} />
                                                    </div>
                                                    <div>
                                                        <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-0.5">Estado Contable</p>
                                        ...
                                        if (vueltoMatch) {
                                            ...
                                            return (
                                                <div className="mt-4 p-5 bg-white border border-gray-200 rounded-(--radius-btn) shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col gap-4">
                                                    <div className="flex items-start gap-3">
                                                        <div className="p-2 bg-black rounded-full shrink-0">
                                                            <Gift size={16} className="text-white" />
                                                        </div>
                                                        <div>
                                                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Acción Requerida</p>
                                                            <p className="text-sm font-bold text-black leading-snug">
                                                                El cliente pagó con ${tenderedAmount} en efectivo. Requiere un vuelto virtual de ${expectedChange}.
                                                            </p>
                                                            <p className="text-xs font-medium text-gray-500 mt-1">
                                                                Cuenta vinculada: <strong className="text-black">{userEmail}</strong>
                                                            </p>
                                                        </div>
                                                    </div>
                                                    
                                                    {/* EL BOTÓN SOLO APARECE AQUÍ */}
                                                    <button 
                                                        onClick={() => setCreditModalOpen(true)}
                                                        className="w-full bg-black hover:bg-gray-800 text-white rounded-xl py-3.5 text-xs font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2 shadow-[0_8px_30px_rgb(0,0,0,0.04)] active:scale-[0.98]"
                                                    >
                                                        <Gift size={14} /> Otorgar Vuelto de ${expectedChange}
                                                    </button>
                                                </div>
                                            );
                                        }
                                    ...
                                    <div className="space-y-2 mb-6">
                                        {selectedOrder.order_items.map((item) => (
                                            <div key={item.id} className="flex justify-between items-center text-sm bg-gray-50 p-3 rounded-(--radius-btn) border border-transparent">
                                                ...
                                                <p className="font-mono font-bold text-gray-900 bg-white px-2 py-1 rounded-(--radius-badge) shrink-0  border border-gray-100">x{item.quantity}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                    ...
                                    return (
                                        <div className="pt-4 border-t border-gray-100 animate-in fade-in">
                                            <div className="flex items-between justify-between mb-3">
                                                <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Comprobante de Retención IVA</label>
                                                <span className="text-[9px] font-black uppercase tracking-widest text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-md">Solo B2B</span>
                                            </div>
                                            <div className="flex gap-2 mb-4">
                                                {[0, 75, 100].map(pct => (
                                                    <button
                                                        ...
                                                        className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all border ${reconcileModal.retentionPct === pct ? 'bg-black text-white border-black shadow-md' : 'bg-gray-50 text-gray-500 border-transparent hover:border-gray-200'}`}
                                                    >
                                                        {pct === 0 ? 'No Aplica' : `${pct}%`}
                                                    </button>
                                                ))}
                                            ...
                                            <div className="bg-gray-50 p-3.5 rounded-xl border border-gray-100 space-y-2">
                                                <div className="flex justify-between text-[11px] font-bold text-gray-500">
                                                    ...
                                                    <span>${Number(targetOrder.total_usd).toFixed(2)}</span>
                                                </div>
                                                {reconcileModal.retentionPct > 0 && (
                                                    <div className="flex justify-between text-[11px] font-bold text-red-500">
                                                        ...
                                                        <span>-${retentionUsd.toFixed(2)}</span>
                                                    </div>
                                                )}
                                                <div className="flex justify-between text-[13px] font-black text-gray-900 pt-2 border-t border-gray-200/50">
                                                    <span>A depositar en Banco:</span>
                                                    <span>${liquidUsd.toFixed(2)}</span>
                                                </div>
                                            </div>
                                        </div>
                                    );
            ...
            {/* 🚀 MODAL DE CRÉDITO DE TIENDA ACTUALIZADO CON PARÁMETROS DE DESVIACIÓN CONTABLE */}
            {selectedOrder && storeId && (
                <IssueVirtualChangeModal
                    ...
                    onSuccess={() => {
                        // 🚀 OPTIMISTIC UI: Mutación local instantánea para destruir el botón y pintar el sello de "Validado"
