...
import { 
  ...
  CreditCard, Copy, AlertCircle, Store, Truck, ChevronRight, Minus, Plus, MapPin, User, ArrowLeft
} from 'lucide-react'
...
export default function FloatingCheckout({ rates, currency, phone, storeName, storeId, storeConfig, products }: CheckoutProps) {
  ...
  const getPaymentButtonStyles = (pm: string, isSelected: boolean) => {
      if (!isSelected) return 'bg-white text-gray-500 border-gray-200 hover:border-gray-300 hover:bg-gray-50'
      switch(pm) {
          case 'Pago Móvil': return 'border-blue-500 text-blue-700 bg-blue-50'
          case 'Zelle': return 'border-purple-500 text-purple-700 bg-purple-50'
          case 'Binance': return 'border-yellow-500 text-yellow-700 bg-yellow-50'
          case 'Zinli': return 'border-rose-500 text-rose-700 bg-rose-50'
          case 'Efectivo': return 'border-emerald-500 text-emerald-700 bg-emerald-50'
          default: return 'border-black text-black bg-gray-50'
      }
  }

  const getPaymentCardStyles = (pm: string) => {
      switch(pm) {
          case 'Pago Móvil': return 'border-blue-200 bg-blue-50/50 text-blue-900'
          case 'Zelle': return 'border-purple-200 bg-purple-50/50 text-purple-900'
          case 'Binance': return 'border-yellow-200 bg-yellow-50/50 text-yellow-900'
          case 'Zinli': return 'border-rose-200 bg-rose-50/50 text-rose-900'
          case 'Efectivo': return 'border-emerald-200 bg-emerald-50/50 text-emerald-900'
                                        ...
                                        <div className="bg-white p-5 rounded-2xl border border-gray-200 space-y-4">
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                                {activePaymentMethods.length > 0 ? activePaymentMethods.map(pm => (
                                                    <button key={pm} onClick={() => setClientData({...clientData, paymentMethod: pm})} className={`px-3 py-3 rounded-xl text-xs font-bold border transition-all duration-200 ${getPaymentButtonStyles(pm, clientData.paymentMethod === pm)}`}>{pm}</button>
                                            ...
                                            {clientData.paymentMethod && getSelectedPaymentDetails() && (
                                                <div className={`mt-2 overflow-hidden rounded-2xl p-5 border animate-in fade-in transition-all ${getPaymentCardStyles(clientData.paymentMethod)}`}>
                                                    <div className="flex justify-between items-start mb-4">
                                                        <div><p className="text-[10px] font-black uppercase tracking-widest mb-1 opacity-60">Datos de {clientData.paymentMethod}</p></div>
                                                        <button onClick={() => handleCopy(getSelectedPaymentDetails())} className="flex items-center gap-1.5 bg-white/60 hover:bg-white/90 border border-black/10 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase transition-colors">{copied ? <Check size={12}/> : <Copy size={12}/>} {copied ? 'Copiado' : 'Copiar'}</button>
                                                    </div>
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="flex gap-3 w-full">
                                    {step === 1 ? (
                                        ...
                                        <button onClick={handleCheckout} disabled={loading} className="flex-1 bg-black text-white px-6 py-3.5 rounded-xl font-bold text-sm hover:bg-gray-800 transition-all active:scale-95 flex items-center justify-center gap-2 border border-black disabled:opacity-70 disabled:cursor-not-allowed">
