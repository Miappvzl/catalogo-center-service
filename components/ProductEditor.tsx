...
export default function ProductEditor({ productId, rates, storeSettings }: ProductEditorProps) {
    ...
    const handleImageUpload = async (files: FileList | File[], target: 'main' | 'productGallery' | 'variant', variantId?: string) => {
        ...
        try {
            ...
            if (target === 'main') {
                ...
                if (variantId) {
                    // Subir foto para una variante ya generada
                    const file = fileArray[0]
                    if (!file.type.startsWith('image/')) throw new Error('Solo imágenes permitidas')
                    const compressedFile = await compressImage(file, 800, 0.7)
                    ...
                    // Subir foto para el generador de variantes
    ...
    const addVariantGroup = () => {
        ...
        setVariantInput({ colorName: '', colorHex: '#000000', defaultStock: 10, priceOverride: '', penaltyOverride: '', images: [] })
        setSizeTags([])
        setSizeInputValue('')
        setIsDirty(true)
    ...
    const handleDeleteProduct = async () => {
        ...
        try {
            ...
            setIsDirty(false);
            Swal.fire({ title: 'Eliminado', text: 'El producto fue borrado.', icon: 'success', confirmButtonColor: '#000', customClass: { popup: 'rounded-[var(--radius-card)]' } });
            router.push('/admin/inventory');
        } catch (error: any) {
            Swal.fire('Error', error.message, 'error');
        } finally {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                gallery: productGallery, // 🚀 Guardamos la galería del padre
    ...
    return (
        <div className="min-h-screen bg-[#F8F9FA] pb-32 font-sans text-gray-900 selection:bg-black selection:text-white overflow-x-hidden w-full max-w-[100vw]">
            ...
            <div className="max-w-4xl mx-auto px-4 md:px-8 py-6 md:py-10 space-y-6">
                ...
                {isDirty && (
                    <div className="bg-yellow-50/80 border border-yellow-200/80 p-4 rounded-[var(--radius-card)] flex items-start sm:items-center gap-4 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="bg-white p-6 md:p-8 rounded-[var(--radius-card)] border border-transparent shadow-sm space-y-6">
                    ...
                    <div className="space-y-5">
                        <div>
                            ...
                            <input value={formData.name} onChange={e => updateForm('name', e.target.value)} placeholder="Ej: Nike Air Force 1" className="w-full bg-gray-50 border border-transparent focus:bg-white focus:border-black focus:shadow-subtle rounded-[var(--radius-btn)] px-4 py-3.5 font-bold text-[16px] md:text-sm text-gray-900 placeholder:text-gray-400 transition-all outline-none" />
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div className="relative" ref={categoryDropdownRef}>
                                ...
                                <input value={formData.category} onChange={e => { updateForm('category', e.target.value); setIsCategoryDropdownOpen(true) }} onFocus={() => setIsCategoryDropdownOpen(true)} placeholder="Ej: Zapatos, Relojes..." className="w-full bg-gray-50 border border-transparent focus:bg-white focus:border-black focus:shadow-subtle rounded-[var(--radius-btn)] px-4 py-3.5 font-bold text-[16px] md:text-sm text-gray-900 placeholder:text-gray-400 transition-all outline-none" />
                                <AnimatePresence>
                                    {isCategoryDropdownOpen && (
                                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute z-50 w-full mt-2 bg-white border border-gray-100 rounded-[var(--radius-card)] shadow-xl overflow-hidden max-h-[240px] flex flex-col">
                                            {existingCategories.filter(c => c.toLowerCase().includes(formData.category.toLowerCase())).length > 0 && (
                                                <div className="p-1.5 overflow-y-auto no-scrollbar">
                                                    ...
                                                    {existingCategories.filter(c => c.toLowerCase().includes(formData.category.toLowerCase())).map(cat => (
                                                        <button key={cat} onClick={(e) => { e.preventDefault(); updateForm('category', cat); setIsCategoryDropdownOpen(false); }} className="w-full text-left px-3 py-2.5 rounded-[var(--radius-btn)] text-sm font-bold text-gray-700 hover:bg-gray-50 hover:text-black transition-colors">{cat}</button>
                                                    ))}
                                                </div>
                                            )}
                                            {formData.category.trim() !== '' && !existingCategories.some(c => c.toLowerCase() === formData.category.trim().toLowerCase()) && (
                                                <div className="p-1.5 border-t border-gray-100 bg-gray-50 shrink-0">
                                                    <button onClick={(e) => { e.preventDefault(); setIsCategoryDropdownOpen(false) }} className="w-full text-left px-3 py-2.5 rounded-[var(--radius-btn)] text-sm font-bold text-black flex items-center gap-2 hover:bg-white hover:shadow-sm border border-transparent hover:border-gray-200 transition-all">
                            ...
                            <div>
                                ...
                                <select value={formData.status} onChange={e => updateForm('status', e.target.value)} className="w-full bg-gray-50 border border-transparent focus:bg-white focus:border-black focus:shadow-subtle rounded-[var(--radius-btn)] px-4 py-3.5 font-bold text-[16px] md:text-sm text-gray-900 transition-all outline-none cursor-pointer appearance-none">
                        ...
                        <div>
                            ...
                            <textarea value={formData.description} onChange={e => updateForm('description', e.target.value)} placeholder="Añade detalles, materiales, cuidados..." className="w-full bg-gray-50 border border-transparent focus:bg-white focus:border-black focus:shadow-subtle rounded-[var(--radius-btn)] px-4 py-3.5 font-medium text-[16px] md:text-sm text-gray-900 placeholder:text-gray-400 min-h-[100px] resize-none transition-all outline-none" />
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 md:p-8 rounded-[var(--radius-card)] border border-transparent shadow-sm space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-start">
                        ...
                        <div className="flex-1 w-full">
                            ...
                            <div className="flex gap-3 h-[120px] md:h-[160px] overflow-x-auto no-scrollbar snap-x pb-2">
                                <button onClick={() => productGalleryInputRef.current?.click()} disabled={productGallery.length >= 3 || uploading} className="w-[120px] md:w-[160px] h-full rounded-[var(--radius-card)] border border-dashed border-gray-300 flex items-center justify-center hover:border-black hover:bg-white transition-all disabled:opacity-50 text-gray-400 hover:text-black shrink-0 bg-gray-50 shadow-sm flex-col gap-2">
                                ...
                                {productGallery.map((img, idx) => (
                                    <div key={idx} className="relative w-[120px] md:w-[160px] h-full rounded-[var(--radius-card)] border border-gray-100 overflow-hidden group bg-white shrink-0 shadow-sm">
                ...
                <div className="bg-white p-6 md:p-8 rounded-[var(--radius-card)] border border-transparent shadow-sm space-y-6">
                    <h3 className="text-lg font-black text-gray-900 border-b border-gray-100 pb-3 flex items-center gap-2"><DollarSign size={20} className="text-gray-400" /> 3. Precio General</h3>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div className="relative group">
                                ...
                                <input type="number" min="0" value={formData.price} onChange={e => updateForm('price', e.target.value === '' ? '' : parseFloat(e.target.value))} placeholder="0.00" className="w-full bg-gray-50 border border-transparent focus:bg-white focus:border-black focus:shadow-subtle rounded-[var(--radius-btn)] pl-8 pr-4 py-3.5 font-black text-xl text-gray-900 outline-none transition-all" />
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="relative group">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold group-focus-within:text-black transition-colors">$</span>
                                <input type="number" min="0" value={formData.penalty} onChange={e => updateForm('penalty', e.target.value === '' ? '' : parseFloat(e.target.value))} placeholder="0.00" className="w-full bg-gray-50 border border-transparent focus:bg-white focus:border-black focus:shadow-subtle rounded-[var(--radius-btn)] pl-8 pr-4 py-3.5 font-bold text-lg text-gray-900 outline-none transition-all" />
                            </div>
                        </div>
                    </div>

                    <div className="bg-[#0A0A0A] rounded-[var(--radius-card)] p-6 text-center relative overflow-hidden mt-4 border border-[#222] shadow-xl">
                ...
                <div className="bg-white p-6 md:p-8 rounded-[var(--radius-card)] border border-transparent shadow-sm">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 border-b border-gray-100 pb-3">
                        ...
                        <div className="flex bg-gray-50 border border-gray-100 rounded-[var(--radius-btn)] p-1 w-full sm:w-auto">
                            <button onClick={(e) => { e.preventDefault(); setHasVariants(false); setIsDirty(true); }} className={`flex-1 sm:flex-none px-4 py-1.5 rounded-[var(--radius-badge)] text-xs font-bold transition-all ${!hasVariants ? 'bg-white text-black shadow-subtle border border-transparent' : 'text-gray-500 hover:text-black border border-transparent'}`}>
                                Producto Único
                            </button>
                            <button onClick={(e) => { e.preventDefault(); setHasVariants(true); setIsDirty(true); }} className={`flex-1 sm:flex-none px-4 py-1.5 rounded-[var(--radius-badge)] text-xs font-bold transition-all ${hasVariants ? 'bg-white text-black shadow-subtle border border-transparent' : 'text-gray-500 hover:text-black border border-transparent'}`}>
                                Con Variantes
                            </button>
                        </div>
                    </div>
                    ...
                    {!hasVariants ? (
                        <div className="bg-gray-50 rounded-[var(--radius-card)] p-6 border border-transparent animate-in fade-in slide-in-from-bottom-2 duration-300 max-w-sm">
                            <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 block ml-1">Stock Disponible</label>
                            <input type="number" min="0" value={simpleStock} onChange={e => { setSimpleStock(e.target.value === '' ? '' : parseInt(e.target.value)); setIsDirty(true) }} className="w-full bg-white border border-transparent focus:border-black focus:shadow-subtle rounded-[var(--radius-btn)] px-4 py-3.5 font-black text-xl text-gray-900 outline-none transition-all shadow-sm" />
                        ...
                        <div className="animate-in fade-in slide-in-from-top-2 duration-300 w-full">
                            ...
                            <div className="bg-gray-50 rounded-[var(--radius-card)] p-5 md:p-6 border border-transparent mb-8 w-full overflow-hidden box-border">
                                ...
                                <div className="space-y-6">
                                    <div className="flex flex-col md:flex-row gap-6">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-3 bg-white p-2 rounded-[var(--radius-btn)] border border-transparent focus-within:border-black focus-within:shadow-subtle transition-all shadow-sm h-[56px]">
                                                {useColor && <input type="color" value={variantInput.colorHex} onChange={e => updateVariantInput('colorHex', e.target.value)} className="w-10 h-10 rounded-[var(--radius-badge)] border-none cursor-pointer bg-transparent shrink-0" />}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                <button onClick={() => variantImageInputRef.current?.click()} disabled={variantInput.images.length >= 3 || uploading} className="w-[56px] h-[56px] rounded-[var(--radius-btn)] border border-dashed border-gray-300 flex items-center justify-center hover:border-black hover:bg-white transition-all disabled:opacity-50 text-gray-400 hover:text-black shrink-0 bg-white shadow-sm">
                                                    {uploading ? <Loader2 className="animate-spin" size={18} /> : <Plus size={22} />}
                                                </button>
                                                {variantInput.images.map((img, idx) => (
                                                    <div key={idx} className="relative w-[56px] h-[56px] rounded-[var(--radius-btn)] border border-gray-100 overflow-hidden group bg-white shrink-0 shadow-sm"><img src={img} className="w-full h-full object-cover" /><button onClick={() => removeImageFromVariantInput(idx)} className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 text-white transition-opacity"><X size={16} /></button></div>
                                                ))}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col md:flex-row gap-6 items-start">
                                        <div className="flex-[2] w-full min-w-0">
                                            ...
                                            <div className="w-full bg-white border border-transparent focus-within:border-black focus-within:shadow-subtle rounded-[var(--radius-btn)] px-3 py-2.5 min-h-[56px] flex flex-wrap items-center gap-2 transition-all shadow-sm">
                                                {sizeTags.map(tag => (<span key={tag} className="flex items-center gap-1.5 bg-black text-white px-2.5 py-1 rounded-[var(--radius-badge)] text-xs font-bold animate-in fade-in">{tag}<button onClick={() => removeSizeTag(tag)} className="hover:text-red-400 transition-colors"><X size={12} /></button></span>))}
                                            ...
                                            <div className="flex flex-wrap gap-1.5 mt-3">
                                                {COMMON_SIZES.map(size => (<button key={size} onClick={(e) => handleQuickPillClick(e, size)} className="px-2 py-1 text-[10px] font-bold bg-white border border-gray-200 text-gray-600 rounded-[var(--radius-badge)] hover:border-black hover:text-black transition-colors active:scale-95 shadow-sm">+ {size}</button>))}
                                            </div>
                                        </div>
                                        <div className="flex-1 w-full min-w-0">
                                            <label className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-2 block">Stock x Variante</label>
                                            <input type="number" min="0" value={variantInput.defaultStock} onChange={e => updateVariantInput('defaultStock', e.target.value === '' ? '' : parseInt(e.target.value))} className="w-full bg-white border border-transparent focus:border-black focus:shadow-subtle rounded-[var(--radius-btn)] px-4 py-3.5 font-black text-xl text-gray-900 outline-none text-center transition-all h-[56px] shadow-sm" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-gray-200">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <div className="relative">
                                                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-sm">$</span>
                                                <input type="number" placeholder="Hereda del producto" value={variantInput.penaltyOverride} onChange={e => updateVariantInput('penaltyOverride', e.target.value)} className="w-full bg-white border border-gray-200 focus:border-black focus:shadow-subtle rounded-lg pl-7 pr-3 py-2.5 text-[16px] md:text-sm font-bold text-gray-900 outline-none transition-all placeholder:font-normal placeholder:text-gray-400" />
                                            </div>
                                        </div>
                                    </div>

                                    <button onClick={addVariantGroup} className="w-full bg-black text-white py-3.5 rounded-[var(--radius-btn)] hover:bg-gray-800 active:scale-[0.98] transition-all font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-subtle border border-black">
                            ...
                            <div className="flex justify-between items-center mb-4 mt-8 px-2">
                                ...
                                <span className="bg-gray-100 border border-gray-200 px-2.5 py-0.5 rounded-[var(--radius-badge)] text-[10px] font-bold text-gray-600">{variants.length} Creados</span>
                            </div>
                            ...
                            {variants.length === 0 ? (
                                ...
                                <div className="space-y-3">
                                    {variants.map((v, i) => (
                                        <div key={v.id || i} className="flex flex-col bg-white border border-gray-100 hover:border-black rounded-[var(--radius-btn)] transition-colors animate-in fade-in slide-in-from-bottom-2 shadow-sm overflow-hidden">
                                            ...
                                            <div className="flex items-center justify-between p-3 md:p-4 cursor-pointer select-none" onClick={() => setExpandedVariantId(expandedVariantId === v.id ? null : v.id)}>
                                                <div className="flex items-center gap-4 min-w-0">
                                                    <div className="w-12 h-12 md:w-14 md:h-14 rounded-[var(--radius-badge)] border border-gray-100 overflow-hidden bg-gray-50 shrink-0 flex items-center justify-center relative">
                                                        {v.variant_image ? <img src={v.variant_image} className="w-full h-full object-cover" /> : <div className="w-full h-full" style={{ backgroundColor: v.color_hex === 'transparent' ? '#f3f4f6' : v.color_hex }}></div>}
                                                    </div>
                                                ...
                                                <div className="flex items-center gap-2 md:gap-4 shrink-0 pl-2">
                                                    ...
                                                    <div className={`p-2 rounded-[var(--radius-badge)] text-gray-500 transition-colors flex items-center gap-1 ${expandedVariantId === v.id ? 'bg-black text-white' : 'hover:bg-gray-100 hover:text-black'}`}>
                                                    ...
                                                    <button onClick={(e) => { e.stopPropagation(); removeVariant(v.id) }} className="p-2 rounded-[var(--radius-badge)] text-gray-400 hover:text-red-500 hover:bg-red-50 border border-transparent transition-colors"><Trash2 size={18} /></button>
                                                </div>
                                            </div>
                                                        ...
                                                        <div className="p-4 md:p-5 space-y-5">
                                                            ...
                                                            <div className="flex flex-col sm:flex-row gap-5">
                                                                <div className="shrink-0 flex flex-col gap-2">
                                                                    ...
                                                                    <button onClick={() => document.getElementById(`file-${v.id}`)?.click()} className="w-16 h-16 rounded-[var(--radius-badge)] border border-gray-200 bg-white hover:border-black flex items-center justify-center overflow-hidden transition-all group shadow-sm">
                                                                        {v.variant_image ? (
                                                                            <img src={v.variant_image} className="w-full h-full object-cover group-hover:opacity-50 transition-opacity" />
                                                                        ) : (
                                                                ...
                                                                <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                                    ...
                                                                    <div className="sm:col-span-2">
                                                                        <label className="text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-1.5 flex justify-between items-center">
                                                            ...
                                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div className="relative">
                                                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-sm">$</span>
                                                                        <input type="number" placeholder="Hereda del producto" value={v.override_usd_penalty ?? ''} onChange={(e) => updateVariantOverride(v.id, 'override_usd_penalty', e.target.value)} className="w-full bg-white border border-emerald-100 focus:border-emerald-500 rounded-lg pl-7 pr-3 py-2 text-[16px] md:text-sm font-bold text-gray-900 outline-none transition-all placeholder:font-normal placeholder:text-gray-400" />
                                                                    </div>
                                                                </div>
                                                            </div>
            ...
            {productId && (
                <div className="max-w-4xl mx-auto px-4 md:px-8 mb-10">
                    <div className="border border-red-200 bg-red-50/30 rounded-[var(--radius-card)] p-6 md:p-8 animate-in fade-in shadow-sm">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                            ...
                            <button onClick={(e) => { e.preventDefault(); handleDeleteProduct(); }} disabled={saving} className="shrink-0 px-6 py-3 bg-white border border-red-200 text-red-600 hover:bg-red-600 hover:text-white hover:border-red-600 font-bold text-sm rounded-[var(--radius-btn)] transition-all active:scale-95 flex items-center justify-center gap-2 shadow-sm"><Trash2 size={18} /> Eliminar Producto</button>
                        </div>
                    </div>
                </div>
            )}
