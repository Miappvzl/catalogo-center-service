...
// =========================================
// EASTER EGG: EL PULSO ARÁCNIDO (BRAND NEW DAY) 🕷️
// =========================================
const SpiderAnomaly = () => {
  // 🚀 CORRECCIÓN: Declaramos 'isGlitching' como el estado único para solucionar el error de TypeScript
  const [isGlitching, setIsGlitching] = useState(false);
  const [showToast, setShowToast] = useState(false);

  const triggerAnomaly = () => {
    if (isGlitching || showToast) return;
    setIsGlitching(true);
    


    // 🚀 AJUSTE: Extendemos el temporizador a 1.8 segundos (1800ms) para que dé tiempo de ver la magia
    setTimeout(() => {
  ...
  return (
    <>
      {/* EL HILO DE SEDA Y LA ARAÑA */}
      <div className="absolute top-0 right-[15%] md:right-[20%] z-[100] flex flex-col items-center">
       <motion.div
          drag="y"
          dragConstraints={{ top: 0, bottom: 0 }}
          dragElastic={0.7} // 🚀 Hilo más elástico y suave
          onDragEnd={(e, info) => {
            // 🚀 Umbral más bajo (con solo tirar 40px hacia abajo se activa)
            if (info.offset.y > 40) {
              triggerAnomaly();
            }
          }}
          onClick={triggerAnomaly} // 🚀 FALLBACK: Si el usuario solo la toca/clica, también funciona
          style={{ touchAction: "none" }} // 🚀 CRÍTICO: Evita que el celular intente hacer scroll al tocarla
          className="flex flex-col items-center cursor-grab active:cursor-grabbing group"
          title="Tira de la telaraña o haz clic..."
        >
          {/* El hilo de telaraña */}
          <div className="w-[1px] h-24 md:h-32 bg-gradient-to-b from-neutral-200 to-neutral-400 group-hover:bg-neutral-600 transition-colors" />
          
          {/* Logo de Spider-Man (WebP sin fondo) */}
          <div className="relative -mt-2 group-hover:scale-110 transition-transform duration-300 drop-shadow-md">
            <img 
              src="/spiderman-logo.webp" 
              alt="Spider" 
              className="w-10 h-10 object-contain pointer-events-none" // pointer-events-none evita bugs de arrastre en la imagen
            />
            {/* Micro-destello rojo de interacción */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 bg-red-500 rounded-full animate-ping opacity-50 pointer-events-none" />
          </div>
        </motion.div>
      </div>

    {/* EL RECONSTRUCTOR DE REALIDAD (Invocación Rúnica Vectorial y Reinicio) */}
      <AnimatePresence>
        {isGlitching && (
          <div className="fixed inset-0 z-[998] pointer-events-none flex items-center justify-center bg-white/5 backdrop-blur-[1px]">
            
           {/* CAPA 1: La Runa Geométrica Ámbar (willChange optimizado para 120FPS) */}
            <motion.div
              ...
              transition={{ duration: 1.8, times: [0, 0.25, 0.75, 1], ease: [0.16, 1, 0.3, 1] }}
            ...
            >
              <svg 
                ...
                style={{ filter: 'drop-shadow(0 0 12px rgba(245,158,11,0.65))' }}
              >
                {/* Círculo exterior rígido */}
                <circle cx="50" cy="50" r="48" fill="none" stroke="currentColor" strokeWidth="0.6" />
                {/* Círculo interno punteado */}
                <circle cx="50" cy="50" r="43" fill="none" stroke="currentColor" strokeWidth="0.8" strokeDasharray="2 3" />
                {/* Estrella geométrica de polígonos cruzados */}
                <polygon points="50,15 78,67 22,67" fill="none" stroke="currentColor" strokeWidth="0.5" />
                <polygon points="50,85 78,33 22,33" fill="none" stroke="currentColor" strokeWidth="0.5" />
                {/* Anillo de runas centrales */}
                <circle cx="50" cy="50" r="18" fill="none" stroke="currentColor" strokeWidth="0.8" />
                <circle cx="50" cy="50" r="5" fill="currentColor" />
                {/* Líneas de ejes y coordenadas */}
                <line x1="50" y1="2" x2="50" y2="98" stroke="currentColor" strokeWidth="0.3" />
                <line x1="2" y1="50" x2="98" y2="50" stroke="currentColor" strokeWidth="0.3" strokeDasharray="1 2" />
              </svg>
            </motion.div>

            {/* CAPA 2: Onda de choque de desenfoque radial */}
            <motion.div
              initial={{ scale: 0, opacity: 0 }}
              animate={{ scale: [0, 1.8, 8], opacity: [0, 1, 0] }}
              transition={{ duration: 1.4, delay: 0.3, ease: "easeOut" }}
              className="absolute w-[180px] h-[180px] rounded-full border-[8px] border-amber-500/25"
              style={{ backdropFilter: "blur(8px) grayscale(100%)" }}
            />

            {/* CAPA 3: El Blinding Flash de sobreexposición (Saturación y Brillo a destello) */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ 
                opacity: [0, 0, 1, 1, 0],
                backdropFilter: [
                  ...
                  "blur(0px) brightness(1)", 
                  "blur(15px) brightness(4) saturate(0)", // Desenfoca y sobreexpone el fondo de la tienda
                  "blur(10px) brightness(4) saturate(0)", 
                  "blur(0px) brightness(1)"
                ]
              }}
              transition={{ duration: 1.8, times: [0, 0.45, 0.75, 0.9, 1], ease: "easeInOut" }}
              className="absolute inset-0 bg-white"
              style={{
                background: "radial-gradient(circle at center, rgba(255,255,255,1) 0%, rgba(255,255,255,0.95) 60%, rgba(255,255,255,0) 100%)",
                boxShadow: "inset 30px 0 60px rgba(239,68,68,0.08), inset -30px 0 60px rgba(59,130,246,0.08)" 
              }}
            />
          </div>
        )}
  ...
  {/* NOTIFICACIÓN DE RECOMPENSA (Onyx Satin Card & Organic Glow) */}
      <AnimatePresence>
        {showToast && (
          <motion.div
            ...
            className="fixed top-20 md:top-24 left-4 right-4 md:left-1/2 md:-translate-x-1/2 md:w-[400px] z-[1000]"
          >
            {/* Contenedor Satinado de Alta Gama */}
            <div className="bg-gradient-to-b from-[#121214] to-[#0A0A0A] backdrop-blur-xl border border-white/[0.08] p-6 rounded-2xl shadow-[0_30px_80px_rgba(0,0,0,0.7)] flex flex-col relative overflow-hidden text-white">
              
              {/* 🚀 CAPA 1: RESPLANDOR AMBIENTAL PROLONGADO (Luz trasera ultra-difusa) */}
              <div className="absolute -top-24 left-1/2 -translate-x-1/2 w-[240px] h-[150px] bg-rose-500/[0.08] blur-[40px] rounded-full pointer-events-none" />
              
              {/* 🚀 CAPA 2: MICRO-FILTRACIÓN DE LUZ EN EL TOP (Luz interna suave) */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 w-4/5 h-[40px] bg-gradient-to-b from-rose-500/[0.12] to-transparent blur-[15px] pointer-events-none" />

              {/* CABECERA DE LA NOTIFICACIÓN */}
              <div className="flex justify-between items-center mb-5 relative z-10">
                <div className="flex items-center gap-2">
                  <div className="w-5.5 h-5.5 rounded-full bg-white/[0.04] border border-white/[0.08] flex items-center justify-center shrink-0">
                    <Sparkles size={11} className="text-rose-400/90" />
                  </div>
                  <span className="text-[9px] font-mono font-bold text-rose-400/90 uppercase tracking-[0.15em] leading-none mt-px">
                    Secreto Desbloqueado
                  </span>
                </div>
                
                {/* Botón de cierre invisible / elegante */}
                <button 
                  onClick={() => setShowToast(false)}
                  className="p-1 text-neutral-500 hover:text-white hover:bg-white/[0.04] rounded-full transition-all active:scale-95"
                >
                  <X size={13} />
                </button>
              </div>
              ...
              <div className="relative z-10 space-y-2 mb-6">
                <h4 className="text-white font-bold text-base md:text-lg tracking-tight leading-tight">
                  Un "Nuevo Día" para tus ventas.
                </h4>
                <p className="text-neutral-400 text-xs md:text-sm leading-relaxed font-medium">
                  Encontraste a nuestro invitado especial por su estreno. Celébralo reclamando tu <span className="text-white font-semibold">primer mes con 50% de descuento</span> en Preziso.
                </p>
              </div>

              {/* BOTÓN DE ACCIÓN (Borde de techa blanca premium) */}
              <a 
                href="https://wa.me/584145811936?text=Hola%20equipo%20Preziso,%20encontr%C3%A9%20la%20ara%C3%B1a%20en%20la%20p%C3%A1gina%20y%20quiero%20reclamar%20mi%20descuento%20como%20nuevo%20usuario%20por%20el%20estreno%20de%20Brand%20New%20Day.%20%F0%9F%95%B7%EF%B8%8F" 
                target="_blank"
