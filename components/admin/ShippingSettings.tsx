...
const FlatToggle = ({ active, label, subtitle, onClick }: { active: boolean, label: string, subtitle?: string, onClick: () => void }) => (
    <div 
        onClick={onClick}
        className={`cursor-pointer p-4 rounded-xl border transition-all flex items-center justify-between active:scale-[0.98] ${active ? 'border-black bg-gray-50' : 'border-gray-200 hover:border-gray-300 bg-white'}`}
    >
        <div className="flex flex-col min-w-0 pr-4">
            <span className="font-bold uppercase text-xs sm:text-sm text-gray-900 truncate">{label}</span>
            {subtitle && <span className="text-[10px] text-gray-500 font-medium uppercase tracking-widest mt-0.5">{subtitle}</span>}
        </div>
        <div className={`w-10 h-6 rounded-full border flex items-center px-1 shrink-0 transition-colors duration-300 ${active ? 'bg-black border-black justify-end' : 'bg-gray-100 border-gray-200 justify-start'}`}>
...
export default function ShippingSettings({ storeId, initialData }: ShippingSettingsProps) {
  ...
  const [config, setConfig] = useState({
    ...
    delivery_zones: [] as {id: string, name: string, cost: number}[] // NUEVO: Zonas de Delivery
  })
  ...
  // --- LÓGICA DE DELIVERY ZONAL (NUEVA) ---
  ...
  return (
    <div className="bg-white p-4 md:p-8 rounded-2xl border border-gray-200 flex flex-col h-full">
        ...
        <div className="flex-1 space-y-8">
            ...
            <div className="space-y-4 pt-6 border-t border-gray-100">
                        ...
                        <div className="bg-gray-50 border border-gray-200 p-5 rounded-xl space-y-4">
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                                <input 
                                    ...
                                    className="flex-1 min-w-0 bg-white border border-gray-200 rounded-lg px-4 py-3 text-sm font-medium focus:ring-1 focus:ring-black focus:border-black outline-none transition-all"
                                    value={newLocation}
                                    onChange={(e) => setNewLocation(e.target.value)}
                                    onKeyDown={(e) => e.key === 'Enter' && addLocation()}
                                />
                                <button onClick={addLocation} className="shrink-0 bg-black text-white px-5 py-3 rounded-lg text-xs font-bold uppercase tracking-wide hover:bg-gray-800 active:scale-95 transition-all flex items-center justify-center gap-2">
                            ...
                            {config.pickup_locations.length > 0 && (
                                <ul className="space-y-2 pt-2">
                                    {config.pickup_locations.map((loc, idx) => (
                                        <li key={idx} className="flex justify-between items-center gap-3 bg-white px-4 py-3 rounded-lg border border-gray-200 text-sm animate-in fade-in duration-200">
                ...
                {/* GESTIÓN DE ZONAS DE DELIVERY (NUEVO) */}
                    ...
                    <div className="pt-2 animate-in slide-in-from-top-2 duration-300">
                        <div className="bg-gray-50 border border-gray-200 p-5 rounded-xl space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <button onClick={addDeliveryZone} className="bg-black text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-gray-800 transition-colors flex items-center gap-1">
                            ...
                            <div className="space-y-3">
                                {config.delivery_zones.length === 0 ? (
                                    <div className="text-center py-4 text-gray-400 text-xs font-bold bg-white rounded-lg border border-dashed border-gray-200">
                                        No has creado ninguna zona tarifada.
                                    </div>
                                ) : (
                                    config.delivery_zones.map((zone) => (
                                        <div key={zone.id} className="flex items-center gap-3 bg-white p-2 rounded-xl border border-gray-200 animate-in fade-in">
                                            ...
                                            <div className="w-24 relative shrink-0 bg-gray-50 rounded-lg border border-gray-200">
                                            ...
                                            <button onClick={() => removeDeliveryZone(zone.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors shrink-0">
                                                <Trash2 size={18}/>
                                            </button>
                                        </div>
                                    ))
