...
export default function ShippingSettings({ storeId }: { storeId: string }) {
  ...
  return (
    ...
    <div className="bg-white p-4 sm:p-6 rounded-xl border border-gray-200 space-y-6 sm:space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-4">
            ...
            <button 
                ...
                className="w-full sm:w-auto bg-[#151515] text-white px-5 py-2.5 rounded-full text-sm font-bold flex items-center justify-center gap-2 hover:bg-gray-800 active:scale-95 transition-all shadow-black/10 disabled:opacity-70"
            >
            ...
            <div className="grid grid-cols-1 min-[400px]:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
                {['mrw', 'zoom', 'tealca'].map((method) => (
                    <div 
                        ...
                        className={`cursor-pointer p-4 rounded-xl border-1 transition-all flex items-center justify-between active:scale-[0.98] ${config.methods[method as keyof typeof config.methods] ? 'border-[#60606057] bg-gray-50' : 'border-gray-100 hover:border-gray-200'}`}
                    >
                        <span className="font-bold uppercase text-sm">{method}</span>
                        <div className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-colors ${config.methods[method as keyof typeof config.methods] ? 'bg-black border-[#60606057]' : 'border-gray-300'}`}>
        ...
        <div className="space-y-4 pt-4 sm:pt-6 border-t border-gray-100">
            ...
            <div 
                ...
                className={`cursor-pointer p-4 rounded-xl border-1 transition-all flex items-center justify-between active:scale-[0.98] ${config.methods.delivery ? 'border-[#60606057] bg-gray-50' : 'border-gray-100 hover:border-gray-200'}`}
            >
                ...
                <div className={`w-10 h-6 rounded-full border flex items-center px-1 shrink-0 transition-colors duration-300 ${config.methods.delivery ? 'bg-black border-[#60606057] justify-end' : 'bg-gray-100 border-gray-200 justify-start'}`}>
                    <motion.div layout className="w-3.5 h-3.5 bg-white rounded-full shadow-sm"/>
                </div>
            </div>

            {/* PICKUP LOCATIONS */}
            <div className={`p-4 rounded-xl border-1 transition-all space-y-4 overflow-hidden ${config.methods.pickup ? 'border-[#60606057] bg-white' : 'border-gray-100 opacity-60'}`}>
                 <div className="flex items-center justify-between cursor-pointer" onClick={() => toggleMethod('pickup')}>
                    ...
                    <div className={`w-10 h-6 rounded-full border flex items-center px-1 shrink-0 transition-colors duration-300 ${config.methods.pickup ? 'bg-black border-[#60606057] justify-end' : 'bg-gray-100 border-gray-200 justify-start'}`}>
                        <motion.div layout className="w-3.5 h-3.5 bg-white rounded-full shadow-sm"/>
                    </div>
                 </div>
