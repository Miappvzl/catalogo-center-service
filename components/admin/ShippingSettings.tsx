...
export default function ShippingSettings({ storeId }: { storeId: string }) {
    ...
    const [config, setConfig] = useState({
        methods: {
            ...
            tealca: false,
            delivery: false,
            pickup: true // Por defecto activado
        },
        pickup_locations: [] as string[], // Lista de direcciones de retiro
        delivery_zones: [] as string[]    // Zonas de delivery (ej: "Zona Norte", "Centro")
    })
    ...
    useEffect(() => {
        const fetchConfig = async () => {
            ...
            if (data?.shipping_config) {
                setConfig(data.shipping_config)
            }
        }
   ...
   const handleSave = async () => {
    if (!storeId) {
        Swal.fire('Error Crítico', 'No se recibió el ID de la tienda', 'error');
        return;
    }
    ...
    console.log("Enviando Configuración:", config, " a tienda ID:", storeId);

    const { error } = await supabase
      .from('stores')
      .update({ shipping_config: config })
      .eq('id', storeId) // <--- Aquí es donde fallaba si el ID era incorrecto
