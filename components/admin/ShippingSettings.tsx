...
import { Truck, MapPin, Save, Loader2, AlertTriangle, Plus, Trash2 } from 'lucide-react'
...
// Componente estandarizado "Flat Toggle" (Sin sombras, solo bordes y color sólido)
...
export default function ShippingSettings({ storeId, initialData }: ShippingSettingsProps) {
  ...
  // Usamos los datos inyectados por el padre para evitar leer la BD de nuevo
  useEffect(() => {
    if (initialData) {
        setConfig(prev => ({
            ...prev,
    ...
    <div className="bg-white p-4 md:p-8 rounded-2xl border border-gray-200 flex flex-col h-full">
        ...
        <div className="flex-1 space-y-8">
            ...
            <div className="space-y-4">
                <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Envíos Nacionales</h4>
            ...
            <div className="space-y-4 pt-6 border-t border-gray-100">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <FlatToggle active={config.methods.delivery} label="Delivery Local" subtitle="Envío en tu ciudad" onClick={() => toggleMethod('delivery')} />
                    <FlatToggle active={config.methods.pickup} label="Retiro Personal" subtitle="El cliente lo busca" onClick={() => toggleMethod('pickup')} />
                </div>
                    ...
                    <div className="pt-4 animate-in slide-in-from-top-2 duration-300">
                        <div className="bg-gray-50 border border-gray-200 p-5 rounded-xl space-y-4">
                            <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest flex items-center gap-1.5">
                                <MapPin size={14}/> Agrega tus puntos de entrega
                            </label>
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                                ...
                                <button 
                                    onClick={addLocation} 
                                    className="shrink-0 bg-black text-white px-5 py-3 rounded-lg text-xs font-bold uppercase tracking-wide hover:bg-gray-800 active:scale-95 transition-all flex items-center justify-center gap-2"
                                >
                                    ...
                                    {config.pickup_locations.map((loc, idx) => (
                                        <li key={idx} className="flex justify-between items-center gap-3 bg-white px-4 py-3 rounded-lg border border-gray-200 text-sm animate-in fade-in duration-200">
                                            ...
                                            <button 
                                                ...
                                                className="shrink-0 text-gray-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-md transition-colors"
        ...
        {/* FOOTER DE ACCIÓN (Idéntico a Identidad) */}
