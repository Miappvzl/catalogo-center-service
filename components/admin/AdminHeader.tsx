...
import { Loader2, ShoppingBag, Camera, Edit2, Copy, Check, Store } from 'lucide-react'
...
export default function AdminHeader({ store, title }: { store: any, title?: string }) {
  ...
  return (
    <header className="px-6 py-6 md:py-8 flex justify-between items-center">
      ...
      {/* CONTENEDOR DE ACCIONES LATERALES */}
      <div className="flex items-center gap-3">
        
        {/* Botones Rápidos (SOLO MÓVIL) */}
        {store?.slug && (
          <div className="flex items-center gap-2 md:hidden">
            <button 
              onClick={copyLink} 
              className="w-10 h-10 rounded-full bg-white border border-gray-200 flex items-center justify-center text-gray-600 active:scale-95 transition-all"
        ...
        <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
          ...
          />
          <div className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-white border-2 border-[rgba(15,15,15,0.9)] flex items-center justify-center overflow-hidden hover:scale-105 transition-all">
            {uploading ? (
          ...
          <div className="absolute -bottom-1 -right-1 bg-black text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
            <Edit2 size={8}/>
          </div>
        </div>
